...
class KmeansImage
{
    ...
    /**
     * @param mixed $image Path to the PNG image
    ...
    public function execute()
    {
        ...
        while(!$converged)
        {
            ...
            foreach($this->imagePoints as $keyPoint => $point)
            {
                $ignore = false;
     ...
     * @return array
     */
    private function generateRGBPointsFromImage($image_file)
    {
        // Create the rgb data
        $imageAsPng = imagecreatefrompng($image_file);

        // Start from the top
        $yCoord = 0;
        $points = array();
        // Then we go down every column
        for(;$yCoord < imagesy($imageAsPng);)
        {

            // Start from the left
            $xCoord = 0;
            // We go across the row and..
            for(;$xCoord < imagesx($imageAsPng);)
            {

                $rgbRaw = imagecolorat($imageAsPng, $xCoord, $yCoord);

                $rgb['r'] = ($rgbRaw >> 16) & 0xFF;
                $rgb['g'] = ($rgbRaw >> 8) & 0xFF;
                $rgb['b'] = $rgbRaw & 0xFF;
